...
def run_prediction_scan(scene_key: str, output_dir: Path, split: str, trials: int | None = None, seed_start: int | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for t_pred in t_preds:
        ...
        for method_key in ["M4", "M7", "M9"]:
            for seed in range(start, start + count):
                ...
                row.update({"t_pred": t_pred, "split": split})
                rows.append(row)
...
def run_phase1_bundle(output_root: Path, split: str, trials: int | None = None, seed_start: int | None = None) -> dict[str, str]:
    start, count = _resolve_split(split, trials, seed_start)
    ...
    for group_key in ["A_basic_modules", "B_encirclement_geometry", "C_pressure_passage"]:
        ...
        run_group(group_key, group_dir, trials=count, seed_start=start)
        produced[group_key] = str(group_dir)

    d1_dir = output_root / "D1_prediction_scan"
    run_prediction_scan("s6_fast_target", d1_dir, split=split, trials=count, seed_start=start)
    produced["D1_prediction_scan"] = str(d1_dir)

    d2_dir = output_root / "D2_prediction_scan"
    run_prediction_scan("d2_fast_target_single_obstacle", d2_dir, split=split, trials=count, seed_start=start)
